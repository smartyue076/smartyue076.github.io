...
## 第一章 开始
1. `endl`将与设备关联的<strong>缓冲区（buffer）</strong>中的内容刷到设备中。在调试时应保证"一直"刷新流。否则若程序崩溃，输出可能还是缓冲区，导致对于程序崩溃位置的错误推断
...
2. 编译(Compilation)
   ...
   这里的编译不是指程序从源文件到二进制程序的全部过程，而是指将经过预处理之后的程序转换成特定<strong>汇编代码(assembly code)</strong>的过程。
